...
class ChatbotService {
  ...
  async chatStream(sessionId, userMessage, onToken) {
    try {
      logger.info("[Chatbot] Starting stream chat with sessionId:", sessionId);
      ...
      logger.info(
        "[Chatbot] Tool result:",
        toolResult
          ? `Found ${Array.isArray(toolResult) ? toolResult.length : 1} items`
          : "No data",
      );
  ...
  async chat(sessionId, userMessage) {
    try {
      logger.info("[Chatbot] Starting chat with sessionId:", sessionId);
      logger.info("[Chatbot] User message:", userMessage);
  ...
  buildContextMessage(userMessage, toolResult) {
    if (
      toolResult &&
      !toolResult.error &&
      (Array.isArray(toolResult) ? toolResult.length > 0 : true)
    ) {
...
[YÊU CẦU]: Giới thiệu ĐÚNG các sản phẩm trên với tên, giá, link chính xác. KHÔNG thêm sản phẩm khác.`;
    ...
    logger.info("[Chatbot] No tool data, using default message");
    return `[KHÁCH HỎI]: ${userMessage}
...
Link mua: ${data.checkoutUrl}`;
  ...
  async detectAndCallTools(message) {
    ...
    if (greetingKeywords.some((k) => lowerMessage.includes(k))) {
      ...
      return await toolHandlers.get_featured_products({
        type: "featured",
        limit: 3,
      });
    }
    ...
    if (categoryKeywords.some((k) => lowerMessage.includes(k))) {
      logger.info("[Chatbot] Detected: categories");
    ...
    if (searchKeywords.some((k) => lowerMessage.includes(k))) {
      ...
      if (keyword.length > 1) {
        logger.info("[Chatbot] Detected: search with keyword:", keyword);
    ...
    if (productRelatedWords.some((w) => lowerMessage.includes(w))) {
      ...
      const foundWord = productRelatedWords.find((w) =>
        lowerMessage.includes(w),
      );
      logger.info("[Chatbot] Detected: product keyword:", foundWord);
      return await toolHandlers.search_products({
        keyword: foundWord,
        limit: 5,
      });
    }
    ...
    if (
      ...
      logger.info("[Chatbot] Fallback: searching with full message");
      return await toolHandlers.search_products({ keyword: message, limit: 5 });
    }

    logger.info("[Chatbot] No tool matched");
    return null;
  }
}
