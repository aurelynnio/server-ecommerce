...
// Real product dataset (open Amazon/Lazada/Shopee/Shein/Walmart samples)
...
function buildRealVariants(item) {
  ...
  return colorsList.map((color, idx) => {
    const priceBump = colorsList.length > 1 ? faker.number.int({ min: -2, max: 8 }) * 10000 : 0;
    ...
    return {
      ...
      sold: item.soldCount || faker.number.int({ min: 0, max: 200 }),
...
function buildRealDescription(item) {
  if (item.description && item.description.trim()) {
    return item.description.replace(/Free Returns ✓ Free Shipping✓\.\s*/gi, '').slice(0, 8000);
  }
  return `Chính hãng ${item.brand || ''} ${item.name}. Sản phẩm chất lượng cao, đóng gói cẩn thận.`;
}

async function seedProducts({ shops, shopCategories, categories, productsPerShop }) {
  if (!realProducts.length) {
    console.error('No real products found. Run build-real-dataset.js first.');
    return;
  }
  const catBySlug = new Map(categories.map((c) => [c.slug, c]));
  const remaining = [...realProducts];
  ...
  for (const shop of shops) {
    ...
    const batch = [];
    const createCount = Math.min(productsPerShop - existing, remaining.length);
    ...
    for (let i = 0; i < createCount; i++) {
      ...
      const variants = buildRealVariants(item);
      const stock = variants.reduce((s, v) => s + v.stock, 0);
      const soldFromVariants = variants.reduce((s, v) => s + v.sold, 0);

      batch.push({
        name: item.name,
        slug: slugify(`real-${item.name}-${crypto.randomBytes(2).toString('hex')}`, {
          lower: true,
          strict: true,
          locale: 'vi',
        }),
        description: buildRealDescription(item),
        shop: shop._id,
