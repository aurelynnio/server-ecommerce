...
describe('Voucher System - Integration Tests', () => {
  ...
  const validateVoucher = (voucher, userId, orderValue, shopId = null) => {
    ...
    if (voucher.endDate && new Date(voucher.endDate) < now) errors.push('Voucher has expired');
    ...
    if (voucher.scope === 'shop' && shopId) {
      ...
      if (voucherShopId !== shopId) errors.push('Voucher is not valid for this shop');
    }
  ...
  describe('Multi-Shop Order with Vouchers', () => {
    it('should apply shop-specific and platform vouchers together', () => {
      ...
      const shopResult = applyVoucher(shopVoucher, 'user1', shopASubtotal, 'shopA');
      ...
      const platformResult = applyVoucher(platformVoucher, 'user1', afterShopDiscount);
