...
describe('ConnectDB Logic', () => {
  ...
  describe('buildMongoUriFromParts', () => {
    ...
    it('should build URI with all parts provided', () => {
      ...
      expect(result).toBe('mongodb://admin:secret@mongo.example.com:27018/mydb?authSource=myauth');
    });
    ...
    it('should produce correct full URI format', () => {
      ...
      expect(result).toBe('mongodb://root:toor@db.local:27019/testdb?authSource=admin');
    });
  });
  ...
  describe('uriFallback', () => {
    ...
    it('should prefer MONGODB_URI when set', () => {
      expect(resolveUri('mongodb://atlas', 'mongodb://local')).toBe('mongodb://atlas');
    });
  ...
  describe('atlasDetection', () => {
    ...
    it('should detect Atlas URI', () => {
      expect(getConnectionType('mongodb+srv://cluster.mongodb.net')).toBe('Atlas (Cloud)');
    });
