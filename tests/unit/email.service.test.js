...
describe("EmailService Logic", () => {
  ...
  describe("subjectSelection", () => {
    ...
    it("should return verification subject for email_verification", () => {
      expect(getSubject("email_verification")).toBe(
        "Verify Your Email Address",
      );
    });
  ...
  describe("verificationLinkConstruction", () => {
    ...
    it("should build correct verification link", () => {
      const result = buildVerificationLink(
        "https://cyhin.engineer",
        "test@example.com",
        "123456",
      );
      expect(result).toBe(
        "https://cyhin.engineer/verify-code?email=test@example.com&code=123456",
      );
    });
    ...
    it("should handle different base URLs", () => {
      ...
      expect(result).toBe(
        "http://localhost:3000/verify-code?email=user@test.com&code=ABCDEF",
      );
    });
  ...
  describe("delegationPattern", () => {
    ...
    const sendPasswordResetCode = (to, code) =>
      sendVerificationCode(to, code, "password_reset");
