  ...
  describe('buildSearchFilter', () => {
    const buildFilter = ({ search, role, isVerifiedEmail }) => {
      ...
      if (search) {
        ...
        filter.$or = [{ username: { $regex: regex } }, { email: { $regex: regex } }];
      }

      if (role) filter.roles = role;
      if (isVerifiedEmail !== undefined) filter.isVerifiedEmail = isVerifiedEmail === 'true';
    ...
    it('should parse isVerifiedEmail as boolean', () => {
      expect(buildFilter({ isVerifiedEmail: 'true' }).isVerifiedEmail).toBe(true);
      expect(buildFilter({ isVerifiedEmail: 'false' }).isVerifiedEmail).toBe(false);
    });
