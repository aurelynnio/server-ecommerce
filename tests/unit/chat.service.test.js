...
describe('ChatService Logic', () => {
  ...
  describe('membershipCheck', () => {
    const isMember = (members, userId) => {
      return members.some((memberId) => memberId.toString() === userId.toString());
    };
  ...
  describe('markAsReadFilter', () => {
    const getUnreadFromOthers = (messages, userId) => {
      return messages.filter((m) => m.senderId.toString() !== userId.toString() && !m.isRead);
    };
  ...
  describe('conversationDedup', () => {
    const findExistingConversation = (conversations, userId, sellerId, shopId) => {
      return conversations.find(
        (c) => c.shopId === shopId && c.members.includes(userId) && c.members.includes(sellerId),
      );
    };
