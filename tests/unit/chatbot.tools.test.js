...
describe("ChatbotTools Logic", () => {
  ...
  describe("searchProductsTransform", () => {
    ...
    it("should handle missing variants", () => {
      const products = [
        { _id: "p1", name: "A", slug: "a", price: { currentPrice: 100 } },
      ];
      const result = transformSearchResult(products);
  ...
  describe("checkAvailability", () => {
    const checkAvailability = (variants, { size, color } = {}) => {
      ...
      return {
        ...
        variants: available,
        message:
          available.length > 0 ? `Còn ${totalStock} sản phẩm` : "Hết hàng",
      };
    };
  ...
  describe("generateCheckoutLink", () => {
    ...
    it("should build checkout URL without variant", () => {
      expect(buildCheckoutUrl("p1", null, 1)).toBe(
        "/checkout?product=p1&quantity=1",
      );
    });

    it("should build checkout URL with variant", () => {
      expect(buildCheckoutUrl("p1", "v1", 2)).toBe(
        "/checkout?product=p1&quantity=2&variant=v1",
      );
    });

    it("should default quantity to 1", () => {
      expect(buildCheckoutUrl("p1", null)).toBe(
        "/checkout?product=p1&quantity=1",
      );
    });

    it("should build add-to-cart URL without variant", () => {
      expect(buildAddToCartUrl("p1", null, 1)).toBe(
        "/cart/add?product=p1&quantity=1",
      );
    });

    it("should build add-to-cart URL with variant", () => {
      expect(buildAddToCartUrl("p1", "v1", 3)).toBe(
        "/cart/add?product=p1&variant=v1&quantity=3",
      );
    });
  });
  ...
  describe("productDetailVariantTransform", () => {
    ...
    it("should handle variant without images", () => {
      const variants = [
        { _id: "v1", size: "S", color: "X", stock: 1, price: {} },
      ];
      const result = transformVariants(variants);
